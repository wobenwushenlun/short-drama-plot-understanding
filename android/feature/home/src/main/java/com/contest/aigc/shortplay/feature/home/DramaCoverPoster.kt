...
internal fun DramaCoverPoster(
    ...
    val localCoverRes = localDramaCoverRes(title = title, imageUrl = imageUrl)
    val imageBitmap by produceState<ImageBitmap?>(initialValue = null, key1 = imageUrl, key2 = localCoverRes) {
        value = if (localCoverRes == null) loadRemoteCoverImage(imageUrl) else null
    }
    ...
    Surface(
        ...
        Box(
            ...
            when {
                localCoverRes != null -> {
                    Image(
                        painter = painterResource(id = localCoverRes),
...
private fun localDramaCoverRes(title: String, imageUrl: String): Int? {
    val key = "$title $imageUrl"
    return if (key.contains("十八岁太奶奶驾到") && key.contains("重整家族荣耀")) {
        R.drawable.tainai3_cover
    } else if (key.contains("北派寻宝笔记")) {
        R.drawable.beipai_cover
    } else if (key.contains("家里家外")) {
        R.drawable.jialijiawai_cover
    } else if (key.contains("幸得相遇离婚时")) {
        R.drawable.xingde_cover
    } else {
        null
    }
}
